...
// Bucket for broker application
resource "aws_s3_bucket" "broker-bucket" {
  bucket = "${var.bucket_prefix}-microfrontends-applications-broker"
...
// Bucket for broker application
resource "aws_s3_bucket_public_access_block" "broker-bucket" {
  bucket = aws_s3_bucket.broker-bucket.id
...
resource "aws_cloudfront_distribution" "broker_distribution" {

  // Origin application
  origin {
    domain_name = aws_s3_bucket.broker-bucket.bucket_regional_domain_name
    origin_id   = "S3-microfrontends-applications-broker"
  }
  ...
  comment             = "TF: Microfrontends (broker App)"
  default_root_object = "index.html"

  logging_config {
    include_cookies = false
    bucket          = aws_s3_bucket.broker-bucket.bucket_regional_domain_name
    prefix          = "cf-logs/"
  }
  ...
  default_cache_behavior {
    ...
    target_origin_id = "S3-microfrontends-applications-broker"
