...
// Bucket for backoffice application
resource "aws_s3_bucket" "backoffice-bucket" {
  bucket = "${var.bucket_prefix}-microfrontends-applications-backoffice"
...
// Bucket for backoffice application
resource "aws_s3_bucket_public_access_block" "backoffice-bucket" {
  bucket = aws_s3_bucket.backoffice-bucket.id
...
resource "aws_cloudfront_distribution" "backoffice_distribution" {
  ...
  // Origin application
  origin {
    domain_name = aws_s3_bucket.backoffice-bucket.bucket_regional_domain_name
    origin_id   = "S3-microfrontends-applications-backoffice"
  }
  ...
  comment             = "TF: Microfrontends (Backoffice App)"
  default_root_object = "index.html"

  logging_config {
    include_cookies = false
    bucket          = aws_s3_bucket.backoffice-bucket.bucket_regional_domain_name
    prefix          = "cf-logs/"
  }
  ...
  default_cache_behavior {
    ...
    target_origin_id = "S3-microfrontends-applications-backoffice"
